...
export const IntelligentImport: React.FC<IntelligentImportProps> = ({
  ...
  return (
    <div className="rounded-xl border border-border bg-elevated/40 p-4">
      <p className="mb-2 text-sm font-medium text-primary">智能导入</p>
      ...
      <div
        ...
        className={`mb-3 flex min-h-[80px] flex-col gap-4 rounded-lg border-2 border-dashed p-4 transition ${
          isDragging ? 'border-accent bg-cyan/5' : 'border-border'
      ...
      >
        ...
        <div className="flex gap-2">
          <textarea
            ...
            className="min-h-[60px] w-full rounded-lg border border-border bg-card/60 px-2 py-1.5 text-sm text-primary placeholder:text-muted"
            value={pasteText}
      ...
      {items.length > 0 && (
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button type="button" className="text-xs text-muted hover:text-primary" onClick={() => toggleAll(true)}>
                全选
              </button>
              <button type="button" className="text-xs text-muted hover:text-primary" onClick={() => toggleAll(false)}>
                取消
              </button>
              <button type="button" className="text-xs text-muted hover:text-primary" onClick={clearAll}>
                清空
              </button>
            </div>
          </div>
            ...
            {items.map((it) => (
              <div
                ...
                className={`flex items-center gap-2 rounded-lg border px-2 py-1.5 text-sm ${
                  it.code ? 'border-border bg-card/60' : 'border-red-500/30 bg-red-500/10'
                }`}
              >
                <input
                  type="checkbox"
                  ...
                  className="rounded"
                />
                <span className={it.code ? 'text-primary' : 'text-red-400'}>
                  {it.code || '解析失败'}
                </span>
