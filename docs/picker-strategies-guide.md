...
| 参数 | 值 | 说明 |
|------|-----|------|
| 60日涨跌幅 | -25% ~ 10% | 跌够多但未完全脱离底部 |
| 当日涨幅 | 0% ~ 5% | 企稳或小幅反弹 |
| 量比 | > 1.2 | 有量能确认 |
| 乖离率 | ≤ 6% | 底部时多为负乖离 |
| 均线 | 不要求 | 底部往往未形成多头 |
| 最大回调 | 100% |  effectively 不设限（底部时 retracement 常接近 100%） |
| 连涨天数 | ≤ 3 | 不追连涨 |
...
| change_60d_max | 无 | 80% | 10% | 50% |
| daily_change_min | -2.5% | 2% | 0% | 0% |
| daily_change_max | 3% | 10% | 5% | 6% |
| volume_ratio_min | 1.0 | 1.5 | 1.2 | 1.0 |
...
| require_ma_bullish | ✓ | ✗ | ✗ | ✗ |
| max_retracement_pct | 50% | 61.8% | 100% | 80% |
...
- 当前四策略均不要求（`require_volume_shrink=False`）
