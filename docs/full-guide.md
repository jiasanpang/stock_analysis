...
| `EMAIL_SENDER_NAME` | 发件人显示名称（默认：stock_analysis股票分析助手） | 可选 |
...
1. **AI 模型**：`AIHUBMIX_KEY`（[AIHubmix](https://aihubmix.com/)，一 Key 多模型）、`GEMINI_API_KEY` 或 `OPENAI_API_KEY`
...
```bash
# 1. 克隆仓库
git clone https://github.com/jiasanpang/stock_analysis.git
cd stock_analysis
...
更多问题请 [提交 Issue](https://github.com/jiasanpang/stock_analysis/issues)
